...
- Public TestFlight distribution and exact-build verification: build 31 is processed and assigned to the public group; the verify-only state query is ready but GitHub Actions is blocked by the account payment or spending limit
...
- 2026-07-12 lead: hardened TestFlight run `29181709447` built, processed, and assigned Machtblick 1.0 build 31 from `3e9ea6e` to the External public group. Fastlane reported successful distribution and the invitation remained live, but App Store Connect did not report `IN_BETA_TESTING` during the complete 30-minute exact-state poll. Added an Ubuntu verify-only dispatch for existing build numbers plus observed processing, external-state, and group-membership evidence, avoiding another binary upload while Apple review state is diagnosed or monitored.
- 2026-07-12 lead: verify-only run `29182688673` selected `ubuntu-latest` correctly but GitHub refused to start any step because recent account payments failed or the Actions spending limit must be increased. The private repository has no self-hosted runner and the local environment has no App Store Connect key, so exact build 31 state verification requires the repository owner to restore GitHub Actions billing before the prepared read-only check can run.
