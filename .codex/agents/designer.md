...
codex_role: worker
source: .claude/agents/designer.md
---

> Generated from `.claude/agents/designer.md` by `scripts/sync-codex-agents.mjs`. Edit the Claude agent and rerun `npm run agents:sync`.
...
The design system in `AGENTS.md` is law. Every mock annotates which tokens it uses; this is what frontend implements against.
...
- Project context (apps, design priorities) is in `AGENTS.md` at the repo root  -  read it.
