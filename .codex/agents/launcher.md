...
codex_role: worker
source: .claude/agents/launcher.md
---

> Generated from `.claude/agents/launcher.md` by `scripts/sync-codex-agents.mjs`. Edit the Claude agent and rerun `npm run agents:sync`.
...
The server MUST outlive you. Do not rely on the Codex harness to manage the process  -  agents die, sessions end, background shells get reaped. Use `nohup` + `disown` so the process is reparented to init and keeps running.
