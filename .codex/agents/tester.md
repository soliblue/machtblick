...
> Generated from `.claude/agents/tester.md` by `scripts/sync-codex-agents.mjs`. Edit the Claude agent and rerun `npm run agents:sync`.
...
- A regression checklist (3-5 flows). If lead didn't provide one, infer from the change and pick the closest neighboring flows.
...
2. **Write the test.** One spec file. Cover the change-under-test first, then the regression flows. Use the project's mobile-emulation viewport (`devices['iPhone 13']`) for any touch-related test, otherwise default desktop. Always:
...
3. **Run it.** Capture pass/fail per case. On failure, capture a screenshot to `/tmp/tester-<slug>-<case>.png`.
4. **Tear down.** Kill the dev server. Delete the spec file unless lead asked you to keep it.
