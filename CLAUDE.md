...
- **LLM work in ETL goes through the `claude` CLI, not the Anthropic API.** We have a Claude subscription; API calls cost money on top. For any enrichment step that needs a model (title simplification, classification, summarization, parsing fallbacks), shell out to `claude -p --model sonnet --output-format json` with the prompt on stdin and parse the JSON `result` field. Loop locally with concurrency control; no SDK, no API keys in code.
