...
function renderThemeOption({
  ...
  return (
    <label
      key={mode}
      className={`relative flex min-w-0 items-center justify-center transition-colors ${expanded ? 'h-[44px] flex-1 gap-s px-m text-m' : 'size-[32px]'} ${value === mode ? 'bg-fg font-semibold text-background' : 'bg-elevated hover:bg-surface'}`}
    >
      <input
...
export function ThemePicker({ value, label, lightLabel, darkLabel, onChange, expanded = false }: Props) {
  ...
  return (
    <fieldset
      role="radiogroup"
      aria-label={label}
      className={`m-0 flex min-w-0 overflow-hidden rounded-m p-0 text-s ${expanded ? 'w-full' : 'w-fit'}`}
    >
      ...
      {[
        ...
        return expanded ? renderThemeOption({ mode, title, Icon, expanded, value, name, onChange }) : (
          <Tooltip key={mode}>
            <TooltipTrigger asChild>
              {renderThemeOption({ mode, title, Icon, expanded, value, name, onChange })}
            </TooltipTrigger>
