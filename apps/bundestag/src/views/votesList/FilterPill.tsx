...
export function FilterPill({ label, options, value, onChange, formatOption, inactiveValue }: Props) {
  ...
  useEffect(() => {
    ...
    const onScroll = (e: Event) => {
      if (e.target instanceof HTMLElement && e.target.contains(pillRef.current)) return
      setOpen(false)
    }
    ...
    <>
      ...
      >
        <button
          ...
          className="flex items-center gap-s before:absolute before:inset-x-0 before:-inset-y-s before:content-['']"
        >
