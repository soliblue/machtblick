...
- 32 px diameter chosen to match the vote-symbol header logo on this same page (CLAUDE.md house convention: 32 for vote-scoped identity). The list-row pattern (36 px in `DefectorRow`, `SpeechRow`) is for full row contexts; an inline strip wants a tick smaller.
