...
On widths <= 360px the gender tile drops the per-slice legend row and shows only the centred largest-slice value; age bucket labels shorten to `<30` / `30 bis 39` / etc. Bar count labels remain.
...
Replace the disc with a faint dashed circle (1px `--color-fg @ opacity-s`, no fill) at the same diameter, centred text inside: `Keine Daten` at `text-s opacity-l`. Legend row hidden. Tile keeps its full height and never collapses.
...
- Bar height: 8px (`h-2`). Track: `--color-fg @ opacity-s`. Fill: single accent (see below). Bar uses `flex-1` so all bars scale to the same max, and the bar with the highest count fills the full track.
...
- Bar fill: party colour via `PARTY_COLOR[party]` from `apps/bundestag/src/lib/parties.ts`. This is the **one** place in the strip where party identity colour is used. It is identity, not decoration, and matches the badge in the table row below.
...
The **shape** of the current filter. A reader who applies "Bundesland: Bayern" should see the gender split and the age skew before counting rows in the table. The strip never tries to be an analysis tool. It is a mirror of the filter row, so the user can trust their filter is doing what they think it is doing.
...
|---|---|---|---|---|
| Tile header label (`GESCHLECHT`, `ALTER`, `PARTEI`) | s uppercase, letter-spacing 0.08em | regular, opacity-l | mb-s | n/a |
| Donut centre count | l | semibold | n/a | n/a |
| Donut centre slice label | s | regular, opacity-l | mt-xs | n/a |
| Donut legend row (`M 348 (55%)`) | s | regular | gap-m, mt-s | n/a |
| Donut legend dot | n/a | n/a | size-2 | n/a |
| Bar row label (`30 bis 39`) | s | regular | n/a | n/a |
| Bar row count | s | semibold | n/a | n/a |
| Tooltip body | s | regular | n/a | Tooltip |
| Empty-state text inside a tile | s | regular, opacity-l | n/a | n/a |
| Tile border | n/a | n/a | 1px | n/a |
