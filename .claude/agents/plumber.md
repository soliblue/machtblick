...
- **Ambiguous Drucksache order:** multi-Drucksache Handzeichen rows do not have one reliable positional rule. When the voted document cannot be inferred safely from the title and protocol, pin its stable vote id in `handzeichen/source-overrides.json`. Proposer resolution, clean titles, and description selection honor that source pin before their normal fallbacks.
...
`etl/bundestag/polarity/self-no-escalate.mjs` selects votes where the initiator's own `position = 'no'` (excluding inverted and procedural) and re-feeds them to the LLM with a **different prompt** (`prompts/etl/bundestag/polarity-self-no.md`): a Fraktion almost never votes against its own Antrag, so self-NO signals a missed Ablehnungs-Beschlussempfehlung whose title is already clean. The prompt receives the official extracted voting block, which outranks proposer and document metadata; an explicit direct vote that rejects the Antrag must not be inverted. On inversion the title stays, only yes/no/result/positions flip. `audit-self-no.mjs` runs after and exits nonzero if any self-NO row remains; `db:validate:votes` also rejects inverted Handzeichen rows whose current initiator is still `no`.
