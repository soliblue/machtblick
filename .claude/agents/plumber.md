...
- **Schema:** `db/schema/` (Drizzle). This is the contract every other agent reads from.
...
- Read `db/schema/`. Extend it, don't replace it without lead's approval.
...
- `names.ts`: `HONORIFICS` + `NAME_PARTICLES` sets for member-name normalization. Every name-key recipe in the repo uses these; drift between local copies caused member-identity forks (plan 101), so never redeclare them.
- `states.mjs` (+ `states.d.mts`): `STATE_BY_CODE` (Liste/Land code → Bundesland).
- `parties.ts`: all party canonicalizers. `matchParty`/`canonicalPartyToken`/`normalizePartyList` + `PARTY_ALIAS_SEED_ROWS`/`PARTY_PATTERNS` (votes vocabulary, re-exported by `db/partyPatterns.ts` for existing import paths), `normalizeFractionLabel` (AW fraction labels, keeps CDU/CSU separate where AW provides them), `normalizeParty` (donation party labels, keeps CDU and CSU separate as legal entities). The three alias tables are deliberately NOT merged: `matchParty` must not resolve bare `CDU`/`CSU` (votes use combined `CDU/CSU`; a bare-CDU alias would change initiator alignment), while terms/donations must keep them separate. All three share one key normalizer (lowercase, umlaut→ae/oe/ue, diacritic strip, soft-hyphen removal, non-alphanumeric→space).
- `awClient.ts`: abgeordnetenwatch API scaffolding. `AW_API`, `AW_UA`, `awJson`, `awText`, `sleep`; the retrying fetch (exponential backoff, 404→null, socket-error retry) is internal.
- `worker.mjs`: scaffolding shared by the LLM workers' `run.mjs` files (`argValue`, `findDbPath`, `chunk`, `sourceHash`, `trimOrNull`, `normalizeDashes`).

`etl/_oneshot/` quarantines applied one-shot scripts kept for provenance: `import-historical.ts` (WP1-15 votes), `ingest-session-81-pdf.ts` (session-81 PDF speech recovery), `bootstrapDipPersons.ts` (initial `members.dip_person_id` seeding), `party-lineage-seed/` (lineage.json seed, safe to re-run if lineages change). Aliased `oneshot:*` in the root package.json. Nothing in `etl/_oneshot/` may be wired into `prompts/auto-refresh.md` or `refresh.mjs`; these scripts never run on scheduled refreshes.
...
- `vote_party_summaries`: per-vote, per-fraction tally. For `namentlich` rows have integer ja/nein/enthalten/absent counts from the roll call. For `handzeichen`/`hammelsprung` only the `position` enum (`yes`/`no`/`abstain`/`mixed`) comes from upstream; the count columns are DERIVED, filled by `db:materialize` (see "Handzeichen party tallies are materialized" below).
...
We compute it from `votes.document` (the free-text Drucksache descriptor). See `db/parseProposingParty.ts`. Patterns we look for, in order:
...
### Handzeichen party tallies are materialized (plan 109)

`vote_party_summaries.members/yes/no/abstain/absent` are populated for `handzeichen`/`hammelsprung` rows by `db/materialize-derived-data.ts > materializeHandzeichenTallies()` (part of `npm run db:materialize`). The values are derived, not upstream: `members` = the party's current seat count, `yes`/`no`/`abstain` = seats when `position` matches else 0, `absent` = 0 (position `mixed` keeps all-zero counts with `members` still set). Seat counts use the inference formerly in the read path's `getSeatsByParty`: first non-null `members` per party across the last 20 current-term namentlich votes (date desc), every party seen in that window collected (plan 110 removed the old stop-at-6 break, which would have silently zeroed a 7th group). Parties without current-term namentlich presence in that window (BSW, FDP labels on handzeichen rows) get 0. The term filter comes from `CURRENT_TERM` (`apps/bundestag/src/server/term.ts`), imported by the materialize script; no hardcoded term literals.

Before plan 109 the app synthesized these numbers at read time in five places (`src/server/{votes,voteDetail,antraege}.ts`, `vite-data/{votes,antraege}.ts`); all now read the stored columns exactly like namentlich rows. Rules that follow:

- `db:materialize` must run after any vote ingest or polarity flip. Both refresh drivers already mandate this. `handzeichen/write.mjs` inserts summary rows with NULL tallies and its upsert only touches `position`; the materialize step re-derives counts from `position` on every run, so re-ingests and polarity position flips self-heal on the next materialize.
- `yes IS NOT NULL` is no longer a namentlich detector. Filter on `votes.vote_type = 'namentlich'` instead. This bit `partyDetail.ts` (cohesion/attendance averages over a `yes != null` list) and was fixed with an explicit vote_type filter; audit any new consumer for the same assumption.
- The seat-count inference lives only in the materialize script; `src/server/seats.ts` keeps only `getChamberSize`.
- **Seatless-party purge (plan 110 round 3):** `materializeHandzeichenTallies()` first DELETEs current-term non-namentlich `vote_party_summaries` rows whose party has no `party_seat_history` row for `CURRENT_TERM` (skipped if seat history for the term is empty, e.g. before `etl:terms`). Backstory: `handzeichen/write.mjs` filters extracted party positions against `member_affiliations` active at the vote date, but the pre-fix affiliations ingest ran over ALL terms' ballots with `term_id` defaulting to 21, so 1,108 WP20-only members carried open "term-21" affiliations and FDP/BSW passed the filter, 13 spurious `position='no', members=0` rows told citizens FDP/BSW voted against 7 WP21 bills (protocol prose mentions those parties, the extractor picks them up). The affiliations ingest is now term-scoped (see below), which fixes the write-side filter, and the materialize purge self-heals any recurrence on every refresh. `party_seat_history` (from `etl:terms`) is the seat-holding source of truth, not `member_affiliations`.

Related dedupe from the same plan: read paths consume the materialized `antraege.abstract_plain` (written by `materializeAntraege()`, same script) instead of stripping HTML from `antraege.abstract` per request.
...
Idempotent: the ingest deletes and rewrites the current term's `member_affiliations` each run.

**Term scoping (plan 110 round 3):** `loadPartyRuns(termId)` only reads ballots on `votes.term_id = CURRENT_TERM`, party labels pass through `canonicalPartyToken` (`etl/_shared/parties.ts`, raw label kept when unmatched so `fraktionslos` survives), and rows are inserted with an explicit `termId`; the delete is `WHERE term_id = CURRENT_TERM`. Before this the ingest ran over ALL terms' vote_members and applied WP21 defaults (`term_id` column default 21, `valid_from = periodStart`), producing 1,108 WP20-only members with open term-21 affiliations, 47 rows with `valid_to < valid_from` (first-run `valid_from` was 2025-03-25 while the next WP20 run started in 2021), and 40 non-canonical `DIE LINKE.` labels. Those ghost rows fed `handzeichen/write.mjs`'s active-party filter and caused the spurious FDP/BSW summary rows (see the seatless-party purge above). We deliberately do NOT generate WP20 affiliation history: there is no boundary data for it, and no read path needs it. Sanity after a run: open term-21 rows must equal the seat total (630) after the chained close-departed script, per-party open counts must match `party_seat_history` term 21.
...
Rule (in `etl/bundestag/votes/import-namentlich.ts > resolveMemberId`): new-member slugs use only the **first whitespace-delimited token** of the first-name component, never the middle names. Hyphenated first names like `Hans-Peter` are kept whole (split on whitespace only). Numeric suffixing on collision (`-2`, `-3`, ...).

On a name-key collision among existing members the resolver prefers term-mandate holders and throws on true ambiguity. In practice the chamber doesn't have two sitting `Mustermann, Max`; if it ever happens, the resolver will need a tiebreaker (wahlperiode + DIP person ID).

The pre-existing Kempf duplicate was merged by a one-shot (`db/merge-kempf.ts`, applied and deleted; git history has it).
...
Critical: `etl/bundestag-affiliations/ingest.ts` is a current-term DELETE + rewrite that reopens every run (`valid_to = null` on each member's last run), so any close-out applied earlier is clobbered by the next affiliations ingest. That is how a superseded one-shot (`db/close-foullong.ts`, since deleted) silently regressed and Baerbock/Habeck + 5 others counted as sitting (637 vs 630) until round 14 caught it. The close script is therefore chained into `npm run etl:affiliations` itself and documented in `prompts/auto-refresh.md` (affiliations step); never run the ingest bare. Sanity signal: the script prints the chamber-wide sitting count (distinct WP21-ballot members with an open affiliation), which must equal the seat total (630).
...
### Normalization (run after every vote ingest, before polarity)

`npm run db:normalize` (script: `db/normalize-results.ts`) flips `result` from `angenommen` to `abgelehnt` for every vote where the proposing party voted `no`. This catches the handzeichen procedural-result cases. Idempotent, but only safe before polarity; see "Order of operations matters" under the initiator extractor.
...
`votes.is_petition_bundle` is true for Sammelübersicht votes, where one plenary vote bundles the committee's per-petition recommendations for dozens of unrelated petitions. `result = angenommen` on these rows means "the bundle of recommendations was accepted", not "every petition won". Individual petitions inside may have been recommended for closure, referral, or rejection. The frontend uses this flag to render a disclaimer banner without title-string-matching at render time. Set at handzeichen ingest by a `Sammelübersicht`/`Petitionsausschuss` title regex in `etl/bundestag/handzeichen/write.mjs`; `db:normalize:reading-pairs` flags remaining rows (the namentlich path) from the same title/document patterns. Backfilled once in migration `0015_votes_is_petition_bundle.sql`.
...
- `^Ablehnung\s+des\s+\w+-?\s*Antrags\b`
- `\bAntrag\b[^.]*?\s+ablehnen\b` / `abzulehnen`
- `^Beschlussempfehlung\b[^.]*?\bablehnen\b`
- `\bEmpfehlung\s+(?:zur|der)\s+Ablehnung\b` / `\bBeschlussempfehlung\s+(?:zur|auf)\s+Ablehnung\b`
...
`etl/bundestag/handzeichen/write.mjs` upserts existing votes: on the `exists` branch it re-derives `vote_party_summaries.position` from raw `v.ja`/`v.nein` and overwrites `votes.result` from raw `v.outcome`. Polarity owns both of those fields for inverted votes. So any handzeichen re-ingest (plan-97 result-repair, the weekly refresh) **reverts the inversion's position flip and result back to the raw procedural orientation while leaving `votes.inverted = 1`**, and because the polarity re-run is gated `WHERE v.inverted = 0`, the row is then skipped forever and never re-flipped. Symptom: an inverted handzeichen vote where the proposing faction's stored `position = 'no'` (it appears to vote against its own Antrag); `db:normalize` accidentally rescues `result` for most but not all, so some also show a wrong `result` (plan 101: 19 votes, 4 with wrong result). Namentlich inverted votes are immune because their ballots come from the XLSX path, which `write.mjs` skips (`if (v.vote_type === 'namentlich') continue`).

Guard (in place since plan 101): `write.mjs` reads `inverted` in its existence probe and, when `inverted = 1`, skips the `result` write and the entire `vote_party_summaries` re-derivation (metadata like title/document/date still refreshes). Do not remove this guard; without it every refresh silently re-breaks these rows. If a genuinely new handzeichen ballot needs to overwrite an inverted row (rare correction), reset `inverted = 0` + delete the `vote_polarity_decisions` row first so polarity re-classifies it cleanly. Rows stranded by pre-guard re-ingests were repaired by one-shots (`db/repair-inverted-handzeichen-ballots.ts` re-flipped `position` yes↔no and recomputed `result` via `apply.mjs::substantiveResultFromSummaries`; `db/repair-inverted-handzeichen-summary.ts` regenerated the DE summaries inversion-aware). Both applied and deleted; git history has them.

Knock-on: the LLM `position_summary`/`key_points` in `vote_party_summaries` (from `etl:party-positions`) are generated against `result`/`position` at the time they ran. If a repair flips positions afterward, those blurbs go stale (they assert "Die X hat … abgelehnt/zugestimmt" in the old orientation). Regenerate with `etl:party-positions -- --vote <id> --vote-type handzeichen --force` then `etl:translations` (hash-keyed, self-heals). The party-positions prompt is not yet inversion-aware, so fix the prompt before a bulk `--force` or it reproduces the procedural/substantive confusion.

### Cron wiring

Chained into `etl/bundestag/handzeichen/refresh.mjs` right after the proposer enrichment step so newly-ingested handzeichen + namentlich votes get classified the same night. Also exposed as `npm run etl:polarity` for ad-hoc runs.
...
`votes.initiator` is the proposing party for substantive votes (one of `CDU/CSU`, `B90/Grüne`, `Die Linke`, `AfD`, `SPD`, `FDP`, `BSW`, `Bundesregierung`, `Bundesrat`, `Sonstige`). Worker: `etl/bundestag/votes/initiator/run.mjs` (script: `npm run etl:initiator`, chained into `handzeichen/refresh.mjs` after polarity and the procedural flagger). Source order: plenarprotokoll XML (authoritative), DIP teaser fallback (`parseProposingParty(document)`), NULL.
...
Backend: `apps/bundestag/src/server/voteDetail.ts` reads `vote_description_decisions.source_pdf_url` for `antragPdfUrl`, so fallback PDFs surface in the "Den vollständigen Antrag findest du hier" callout.
...
`prompt.mjs` exports `PROMPT_VERSION` (currently `6`). Bump it whenever you edit the prompt. The runner re-generates only rows where `vote_description_decisions.prompt_version != PROMPT_VERSION` (or no decision row). No need to manually delete rows.
...
## Bundestag speeches: data notes

Speeches come from the official Plenarprotokoll XML via `etl/bundestag-reden-xml/` (`npm run etl:speeches:xml`). The original CPP-BT parquet path (`etl/bundestag-reden/`, Zenodo `10.5281/zenodo.4542661`, alias `etl:speeches`) was superseded by the XML ingest and deleted in plan 109; git history has the code and the CPP-BT quirk notes. Speaker→member matching uses the shared name normalization (`etl/_shared/names.ts` honorifics/particles) plus `members.bt_mdb_id` from Stammdaten; government-role speakers (`speaker_role` set) are intentionally not matched to a member.
...
`vote_debate_groups` is materialized by `db:materialize` (`db/materialize-derived-data.ts`, function `materializeVoteDebateGroups`, full DELETE+rebuild, idempotent). **Both** of its linkage paths require `votes.agenda_item` to be set:
...
```
backfillAgendaItem.ts   (sets votes.agenda_item from protocol XML)
...
### `etl/_oneshot/party-lineage-seed/` (logical party identities + transition events)

Hand-curated `lineage.json`. ~14 lineages, ~8 events. Idempotent (upserts lineages, replaces members + events). Run before `etl:terms` so `party_seat_history.lineage_id` resolves on first ingest. Alias `oneshot:lineage-seed`; applied, only re-run when lineages change.
...
- `antrag_signatories`: `(antrag_id, member_id)` composite PK. Resolved via `aktivitaet.person_id → members.dip_person_id`. No role/order column; DIP doesn't expose lead-vs-co-signer.
- `antraege_raw`: sidecar with full vorgang + positions JSON, matches the votes raw pattern.
...
`buildSignatories.ts > buildSignatoryRows` emits `(targetId, memberId)` candidate rows; `process.ts` keeps only those whose `targetId` is in `antraege`.
...
Membership filter (`antragIds.has(tid)`) does the final selection, non-target vorgangsbezug entries (e.g. the EU-Vorlage) drop because they're not in `antraege`. Both fixes are required: the pre-filter prevented the aktivitaet from reaching `buildSignatoryRows` in the first place.
...
**Adding party patterns.** `etl/_shared/parties.ts` is the single source of truth for fraction-name normalization (`db/partyPatterns.ts` re-exports it), shared between `parseProposingParty.ts` and `initiatorAligns.ts`. When a novel `votes.initiator` form or `initiative_fraktion` upstream label appears, add a regex there, both consumers pick it up.
...
Re-running `npm run etl:portraits` updates every matched row with the freshest URL + author + license. Members that lose their match (e.g. Wikidata image deleted) will retain stale data. The script does not null out rows that no longer match. If we ever need to invalidate, do it via one-shot SQL. Applied once (plan 110 round 3): `mast-katja`, Commons file deleted upstream (404) and Wikidata P18 removed, all four `picture_*` columns nulled so the initials fallback renders; a portraits re-run stays NULL because unmatched members are never touched.
